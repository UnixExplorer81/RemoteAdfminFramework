function ExecuteApiRequest {
    ...
    if ($current -isnot [array] -or $current.Count -lt 2) {
        $Context.Logger.Error("ExecuteApiRequest: No executable job on path $($Path -join ' > ')")
        return
    }
    ...
    for ($j = 2; $j -lt $current.Count; $j++) {
        ...
        if ($e -is [System.Collections.Hashtable] -or $e -is [System.Collections.Specialized.OrderedDictionary] -and $e.Keys -contains 'Description' -and $e.Keys -contains 'Script') {
            ...
            $Context.Logger.Error("ExecuteApiRequest: Invalid job entry: $e")
            return
        }
    }
    ...
    if ($entryPoint -is [ScriptBlock]) {
        ...
        $Context.Logger.Error("ExecuteApiRequest: The entry point is not a script block")
        return
    }
